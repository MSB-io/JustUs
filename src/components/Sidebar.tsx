...
const Sidebar: React.FC<SidebarProps> = ({ currentPage, setCurrentPage, isCollapsed }) => {
  return (
    <aside className={cn(
      ...
      <div className="flex flex-col flex-grow justify-center py-4 px-2">
        ...
        <nav className="flex flex-col gap-1">
          {sidebarLinks.filter(link => link.name !== 'Upgrade').map((link) => {
        ...
        <div className="flex flex-col gap-1">
          ...
          >
            ...
            {!isCollapsed && <span className="font-semibold whitespace-nowrap">New playlist</span>}
          </button>
          {userPlaylists.map((playlist) => (
            ...
            >
              ...
              {!isCollapsed && <span className="text-sm font-medium truncate whitespace-nowrap">{playlist}</span>}
            </a>
          ))}
        </div>
      </div>
