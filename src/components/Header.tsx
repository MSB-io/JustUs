...
import { ChevronLeft, ChevronRight, Search, Bell, User, PanelLeftClose, PanelRightClose } from 'lucide-react';
...
const Header: React.FC<HeaderProps> = ({ isCollapsed, setIsSidebarCollapsed }) => {
  return (
    <header className="flex items-center justify-between p-4 bg-background/80 backdrop-blur-sm border border-border rounded-xl">
      <div className="flex items-center gap-4">
        ...
        >
          {isCollapsed ? <PanelRightClose className="h-6 w-6" /> : <PanelLeftClose className="h-6 w-6" />}
        </button>
        <Logo />
